...
if (!function_exists('template')) {
    function template($asset = false)
    {
        try {
            ...
            $activeTheme = ($prefix === '/user' && $last !== 'check')
    ...
    function renderHeaderMenu($menuItems)
    {
        ...
        foreach ($menuItems as $menuItem) {
            if (isset($menuItem['child'])) {
                ...
                echo '<a class="nav-link text-uppercase ' . (ucfirst(getLastSegment()) == $menuItem['name'] ? 'active' : '') . '"  href="' . $menuItem['route'] . '">' . trans($menuItem['name']) . '</a>';
            }
            echo '</li>';
        }
        echo '</ul>';
    ...
    try {
        ...
        $pageDetails = PageDetail::select('id', 'page_id', 'name')
            ...
            ->whereHas('page', function ($query) use ($name) {
                $query->where('name', $name)->where('template_name', basicControl()->theme);
            })
            ->first();
...
if (!function_exists('getPwaData')) {
    function getPwaData()
    {
        ...
        $singleContent = $pwaPopupContent
            ...
            ->where('content.theme', basicControl()->theme)
            ->first();
