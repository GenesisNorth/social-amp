...
class PaymentLogController extends Controller
{
    ...
    public function index()
    {
        ...
        if (!$paymentRecord) {
            $paymentRecord = Deposit::whereNull('child_panel_id')->selectRaw('COUNT(id) AS totalPaymentLog')
    ...
    public function search(Request $request)
    {
        ...
        return DataTables::of($deposit)
            ...
            ->addColumn('amount', function ($item) {
                ...
                return '<h6 class="mb-0 ' . $statusClass . '">' . fractionNumber(getAmount($item->amount)) . ' ' . $item->payment_method_currency . '<span class="badge bg-soft-secondary text-dark ms-1" data-bs-toggle="tooltip" data-bs-placement="top" title="Charge">+ ' . getAmount($item->percentage_charge) + getAmount($item->fixed_charge) . " " . $item->payment_method_currency . '</span>' . '</h6>';
            })
            ...
            ->addColumn('action', function ($item) {
                ...
                data-amount='" . currencyPosition(getAmount($item->payable_amount_in_base_currency)) . "'
            ...
            ->rawColumns(['name', 'method', 'amount', 'payable', 'status', 'action'])->make(true);
    }
    ...
    public function paymentRequest(Request $request)
    {
        ...
        return DataTables::of($funds)
            ...
            ->rawColumns(['name', 'method', 'amount', 'payable', 'status', 'action'])
            ->make(true);
    }
    ...
    public function action(Request $request, $id)
    {
        ...
        if ($request->status == '1') {
            ...
            $action = [
                "link" => 'user.fund.index',
                "icon" => "fa-light fa-bullhorn"
            ];

            $this->userPushNotification($data->user, 'PAYMENT_APPROVED', $msg, $action);
            $this->userFirebasePushNotification('PAYMENT_APPROVED', $msg, route('user.fund.index'));
            ...
            $this->userFirebasePushNotification('PAYMENT_REJECTED', $msg, $action, route('user.fund.index'));
