...
class SupportTicketController extends Controller
{
    ...
    public function tickets($status = 'all')
    {
        ...
        if (!$ticketRecord) {
            $ticketRecord = SupportTicket::whereNull('child_panel_id')->selectRaw('COUNT(id) AS totalTicket')
    ...
    public function ticketSearch(Request $request)
    {
        ...
        $supportTicket = SupportTicket::query()->whereNull('child_panel_id')->with('user:id,username,firstname,lastname,image,image_driver')
            ->whereHas('user')
            ->orderBy('id', 'DESC')
    ...
    public function ticketReplySend(Request $request, $id): \Illuminate\Http\RedirectResponse
    {
        $request->validate([
        ...
        try {
            ...
            $msg = [
                ...
                'reply' => $resTicketDetails->message,
            ];
            $action = [
