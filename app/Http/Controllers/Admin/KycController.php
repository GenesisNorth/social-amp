...
class KycController extends Controller
{
    ...
    private function validateRequest($request)
    {
        ...
        $customMessages = [
            ...
            'is_required.*.in' => 'The selected required value is invalid.',
        ];
    ...
    public function view($id)
    {
        try {
            $data['userKyc'] = UserKyc::with('user')->where('id', $id)->firstOr(function () {
                throw new Exception('The user KYC record was not found.');
            });
    ...
    public function userSendMailNotify($user, $type): void
    {
        if ($type == 'approve') {
        ...
        $action = [
            "link" => route('user.kyc.verification'),
            "icon" => "fa-light fa-bullhorn"
        ];
        $this->sendMailSms($user, $templateKey);
        $this->userPushNotification($user, $templateKey, $msg, $action);
        $this->userFirebasePushNotification($user, $templateKey, $msg, '#');
    }
}
