...
class UsersController extends Controller
{
    ...
    public function setServiceRate(Request $request)
    {
        ...
        $userServiceRate = UserServiceRate::firstOrNew([
            ...
            'price_percentage' => $request->percentage ?? 0
        ]);
        ...
        $result = $user->serviceRates->map(function ($item) {
            return [
                ...
                'service' => [
                    ...
                    'service_title' => optional($item->service)->service_title,
                    'price' => optional($item->service)->price,
                    'provider_name' => optional($item->service)->provider_name
                ]
            ];
        });
    ...
    public function updateServiceRate(Request $request)
    {
        ...
        $data = UserServiceRate::with('service')->find($request->id);

        if ($request->percentage > 0) {
            $originalPrice = optional($data->service)->price;
            $updatePrice = ($originalPrice * $request->percentage) / 100;
            $data->price_percentage = $request->percentage;
            $data->price = $updatePrice;
        } else {
            $data->price_percentage = 0;
