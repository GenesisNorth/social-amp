...
class Payment
{
    public static function prepareData($deposit, $gateway)
    {
        ...
        $val['Method'] = 'GET';
        $val['RespUrl'] = route('ipn', [$deposit->child_panel_id ? $gateway->gateway->code : $gateway->code, $deposit->trx_id]);

        $CancelUrl = $deposit->child_panel_id ? route('child.panel.user.payment.request') : route('user.payment.request');
