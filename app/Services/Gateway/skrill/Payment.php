...
class Payment
{
    ...
    public static function prepareData($deposit, $gateway)
    {
        ...
        $val['cancel_url'] = $deposit->child_panel_id ? route('child.panel.user.fund.index') : route('user.fund.index');
        $val['status_url'] = $deposit->child_panel_id ? route('ipn', [optional($gateway->gateway)->code, $deposit->trx_id]) : route('ipn', [$gateway->code, $deposit->trx_id]);
        $val['language'] = 'EN';
