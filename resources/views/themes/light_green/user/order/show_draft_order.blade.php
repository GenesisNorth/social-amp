    ...
    <div class="dashboard-wrapper">
        <div class="container">
            <div class="breadcrumb-area">
                <h4 class="title">@lang("Draft Mass Order")</h4>
                <ul class="breadcrumb">
            ...
            <form action="{{ route('user.mass.order.store') }}" method="post">
                ...
                <div class="card mt-50">
                    <div class="card-header d-flex justify-content-between align-items-center border-0">
                        <h4 class="mb-0">@lang("Draft Mass Order")</h4>
                        <div class="btn-area">
                            <a href="{{ route('user.mass.order') }}" class="cmn-btn"><i
                                    class="fa-sharp fa-light fa-backward"></i> @lang("Back")
                            </a>
                        </div>
                    </div>
                    <div class="card-body">
                        @if(count($draftMassOrders) > 0)
                            <div class="cmn-table">
                                            ...
                                            <input type="text" class="form-control service_id"
                                                ...
                                                <td data-label="Price">
                                                    @if(auth()->user()->currency)
                                                        <span>
                                                    {{ currencyPositionBySelectedCurrency($draft->price * ($currency ? $currency->conversion_rate : 1), auth()->user()->currency) }}
                                                </span>
                                                    @else
                                                        <span>
                        ...
                        @else
                            <div class="text-center p-4">
                                ...
                                <p class="mb-0">@lang("No draft orders available to display.")</p>
                            </div>
                        @endif
                    </div>
                </div>
